...
function DevDiagnostics() {
  ...
  return (
    <div className="bg-[#0b0c10] border-t border-red-500/20 py-6 text-center w-full z-[9999]">
      <div className="max-w-4xl mx-auto px-6 text-[11px] font-mono text-red-400 space-y-2">
        <p className="font-bold text-xs text-red-500 uppercase tracking-widest">⚠️ Development Diagnostics Panel</p>
        <p>Vite Environment Supabase URL: <span className="text-white">{dbStatus.url}</span></p>
        <p>Active URL (proxied path): <span className="text-white">/supabase</span></p>
        {dbStatus.loading ? (
          <p className="animate-pulse">Checking articles count in database...</p>
        ) : dbStatus.error ? (
          <p className="text-red-500 font-bold">❌ Connection Error: {dbStatus.error}</p>
        ) : (
          <p>✅ Connection Successful! Articles loaded by browser: <span className="text-white font-bold">{dbStatus.articlesCount}</span></p>
        )}
      </div>
    </div>
  );
